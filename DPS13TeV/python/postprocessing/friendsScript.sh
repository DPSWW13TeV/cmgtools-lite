...
echo running command: python ${1}/src/CMGTools/DPS13TeV/python/postprocessing/postproc_batch.py ./ ${localOutDir} -d ${3} -N ${5} -c ${4} --friend


mkdir -p ${localOutDir}
python ${1}/src/CMGTools/DPS13TeV/python/postprocessing/postproc_batch.py ./ ${localOutDir} -d ${3} -N ${5} -c ${4} --friend
...
cp ${PWD}/${localOutDir}/tree_Friend_${3}*.root ${6}/
