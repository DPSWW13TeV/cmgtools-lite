...
if __name__ == '__main__':
    ...
    for isd,sd in enumerate(os.listdir(options.directory)):
        ...
        tmp_f = fp+'/treeProducerSusyMultilepton/tree.root' ##org
        #tmp_f = fp#+'_treeProducerSusyMultilepton_tree.root'
        print tmp_f
        ...
        try:
            ...
            print 'file name is %s' % (tmp_f)
            tmp_f = ROOT.TFile(tmp_f, 'READ')
            tmp_t = tmp_f.Get('tree')#tree
            tmp_n = tmp_t.GetEntries()
            print 'got the tree with {enteries}'.format(enteries=tmp_n)
            dss[sd] = tmp_n
