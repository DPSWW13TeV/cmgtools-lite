...
class MCAnalysis:
    def __init__(self,samples,options):
        ...
        if ',' in options.year:
            ...
            for year,lumi in zip(years,lumis):
                ...
                for pname,tty in self._subMcas[-1]._allData.items():
                ...
                for key, issig in self._subMcas[-1]._isSignal.items():
                    if key not in self._isSignal: self._isSignal[key] = issig
                for key, rank in self._subMcas[-1]._rank.items():
                    self._rank[key] = rank + rankoffset
                for key, opt in self._subMcas[-1]._optionsOnlyProcesses.items():
        ...
        if options.checkFriendsFirst:
            ...
            ok = True
            for ttys in self._allData.values():
                for tty in ttys:
    ...
    def readMca(self,samples,options,addExtras={},selectProcesses=None):
        ...
        for line in samples:
            ...
            field = [field_previous[i] if field[i]=="$prev" else field[i] for i in range(len(field))]
           ...
           ## If we have a user-defined list of processes as signal
            ...
            for cname in cnames:
                ...
                for rootfile in rootfiles:
                    ...
                    tty = TreeToYield(rootfile, basepath, options, settings=extra, name=pname, cname=mycname, objname=objname, variation_inputs=list(variations.values()), nanoAOD=(treename == "NanoAOD")); 
                    tty.pckfile = pckfile
                    ttys.append(tty)
    ...
    def listProcesses(self,allProcs=False):
        ...
        if 'data' in list(self._allData.keys()): ret.append('data')
        return ret
    def listOptionsOnlyProcesses(self):
        return list(self._optionsOnlyProcesses.keys())
    ...
    def listSignals(self,allProcs=False):
        ret = [ p for p in list(self._allData.keys()) if p != 'data' and self._isSignal[p] and (self.getProcessOption(p, 'SkipMe') != True or allProcs) ]
        ret.sort(key = lambda n : self._rank[n])
        return ret
    def listBackgrounds(self,allProcs=False):
        ret = [ p for p in list(self._allData.keys()) if p != 'data' and not self._isSignal[p] and (self.getProcessOption(p, 'SkipMe') != True or allProcs) ]
        ret.sort(key = lambda n : self._rank[n])
        return ret
    ...
    def prepareForSplit(self):
        ...
        if len(fname2tty) and len(fname2entries):
            for fname,entries in fname2entries.items():
                if fname not in fname2tty: continue
                for tty in fname2tty[fname]: tty.setEntries(entries)
                del fname2tty[fname]
        if len(fname2tty):
            retlist = self._processTasks(_runGetEntries, [(k,v[0]) for (k,v) in fname2tty.items()], name="GetEntries")
    ...
    def prettyPrint(self,reports,makeSummary=True):
        ...
        allSig.sort(key = lambda n_v: self._rank[n_v[0]])
        allBg.sort( key = lambda n_v1: self._rank[n_v1[0]])
        table = allSig + allBg
        if makeSummary:
            if len(allSig)>1:
        ...
        for fomname in self._options.figureOfMerit:
            ...
            nrows = len(table[0][1])
            table += [ (fomname, [ (None, [fom(self, reports, row), 0, 0]) for row in range(nrows) ] ) ]
        ...
        if self._options.txtfmt == "text":
            ...
            if self._options.txtfmt in ("md","jupyter"):
                ...
                ret.insert(1,(("---:" if i else "---") for i in range(len(headers))))
    ...
    def _processTasks(self,func,tasks,name=None,chunkTasks=200,verbose=False):
        ...
        if self._options.jobs == 0: 
            retlist = list(map(func, tasks))
        else:
            from multiprocessing import Pool, cpu_count
            retlist = []
            for i in range(0,len(tasks),chunkTasks):
                ...
                del pool
        if verbose:
            print("Done %s in %s s at %.2f " % (name,timer.RealTime(),0.001*(int(ROOT.gSystem.Now()) - _T0)))
        return retlist
    def _splitTasks(self,tasks):
        ...
        if not self._options.splitDynamic:
            for task in tasks:
                for fsplit in [ (i,nsplit) for i in range(nsplit) ]:
            ...
            for task in tasks:
                ...
                fsplits = [ (i,chunks) for i in range(chunks) ]
    ...
    def _normalizeGroups(self):
        ...
        for (igroup,total_w) in mergemap.items():
