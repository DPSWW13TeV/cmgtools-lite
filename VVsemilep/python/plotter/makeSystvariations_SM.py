...
def gethists(fName,NPs,proc,odir):
    ...
    for np in NPs:
        ...
        h_eft_nom=filetoread.Get('%s_%s'%(varname,proc));h_nom.SetDirectory(0)
        h_eft_up=filetoread.Get('%s_%s%sUp'%(varname,proc,np));h_up.SetDirectory(0)
        h_eft_dn=filetoread.Get('%s_%s%sDown'%(varname,proc,np)); h_dn.SetDirectory(0)

        h_smeft_nom=filetoread.Get('%s_%s'%(varname,proc));h_nom.SetDirectory(0)
        h_smeft_up=filetoread.Get('%s_%s%sUp'%(varname,proc,np));h_up.SetDirectory(0)
        h_smeft_dn=filetoread.Get('%s_%s%sDown'%(varname,proc,np)); h_dn.SetDirectory(0)
        
        makeCanvas(h_nom,h_up,h_dn,odir,proc,np)
    filetoread.Close();
    return True

def getratios(h_nom,h_up,h_dn):
     h_ratioU = h_up.Clone();
     h_ratioD = h_dn.Clone();
     h_ratioU.Divide(h_nom); h_ratioD.Divide(h_nom);
     h_ratioU.etDirectory(0);
     h_ratioD.etDirectory(0);
     return h_ratioU,h_ratioD
