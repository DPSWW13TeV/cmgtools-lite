...
if "/fakeRate_cc.so" not in ROOT.gSystem.GetLibraries():
    ROOT.gROOT.ProcessLine(
        ".L %s/src/CMGTools/VVsemilep/python/plotter/fakeRate.cc+" % os.environ['CMSSW_BASE'])
            ...
            for line in stream:
                ...
                if fields[0] == "weight":
                    if self._weight is not None:
                        raise RuntimeError(
                            "Duplicate weight definition in fake rate file "+file)
                    self._weight = fields[1]
                elif fields[0] == "change":
                    self._mods.append(SimpleCorrection(
                        fields[1], fields[2], alsoData=True))
                elif fields[0] == "cut-change":
                    self._cutMods.append(SimpleCorrection(
                        fields[1], fields[2], onlyForCuts=True, alsoData=True))
                elif fields[0] == "alt-norm":
                    # this only does something when the fr is part of an unc file
                    self._altNorm = fields[1]
                    ...
                    if len(fields) < 5 or year == None or year in fields[4].split(','):
                        if loadFilesNow:
                            ...
                            self._toLoad.append(
                                (fields[1], fname, hname, file))
                elif fields[0] == 'norm-lumi-override':
                    if self._weight is None:
                        raise RuntimeError(
                            "norm-lumi-override must follow weight declaration in fake rate file "+file)
                    if not lumi:
                        raise RuntimeError(
                            "lumi not set in options, cannot apply norm-lumi-override")
                    print("WARNING: normalization overridden from %s/fb to %s/fb in fake rate file %s" %
                          (lumi, fields[1], file))
                    self._weight = '((%s)*(%s)/(%s))' % (self._weight,
                                                         fields[1], lumi)
                elif fields[0] == 'cut-file':
                    if self._weight is None:
                        raise RuntimeError(
                            "cut-file must follow weight declaration in fake rate file "+file)
                    addcuts = CutsFile(
                        fields[1], options=None, ignoreEmptyOptionsEnforcement=True)
                    self._weight = '((%s)*(%s))' % (self._weight,
                                                    addcuts.allCuts(doProduct=True))
#                    print "WARNING: cuts loaded from fake rate file "+file
            ...
            if file == files[0]:
                if self._weight is None:
                    raise RuntimeError(
                        "Missing weight definition in fake rate file "+file)
    ...
    def _loadFile(self, hist, fname, hname, txtfilename):
        if hist in _loads:
            if _loads[hist][0] != (fname, hname):
                print("Conflicting load for %s: (%r, %r) from %r vs older %r from %r" % (
                    hist, fname, hname, txtfilename, _loads[hist][0], _loads[hist][1]))
            else:
