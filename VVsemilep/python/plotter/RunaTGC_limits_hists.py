...
def commandsToRun(dc,pf,plots_odir,WC):
    ...
    os.system("combine  -M FitDiagnostics  model_{name}.root  -t -1 --redefineSignalPOIs k_{op} --freezeParameters r,k_{op} --cminDefaultMinimizerStrategy 0 --setParameters r=1,k_{op}=0".format(name=dCard_str,op=WC)) #--saveNormalizations  --saveShapes --plots  # --robustFit=1  --toysFrequentist 
