...
class Prepare_workspace_4limit:
		...
		hists4scale['c_int_cw_cb_%s_hist'%WV].Sumw2(kTRUE)
			...
			#cw-SM interference
			hists4scale['c_dif_%s_hist_cw'%WV].Fill(MWW,0.5*(aTGC[72]-aTGC[52]) * weight_part)
			#cb-SM interference
			hists4scale['c_dif_%s_hist_cb'%WV].Fill(MWW,0.5*(aTGC[64]-aTGC[60]) * weight_part)
			#cwww-SM interference
			hists4scale['c_dif_%s_hist_cwww'%WV].Fill(MWW,0.5*(aTGC[112]-aTGC[12]) * weight_part)
			#cwww+cw
        ...
        def Make_plots(self,rrv_x,cat,fitres):
            ...
            ##amfor i in range(3):
            ##am    rrv_x.setRange(self.binlo,self.binhi)
            ##am    c1  = TCanvas("AAC"+cat+'_'+self.POI[i]+'-',self.POI[i]+'-',1)
            ##am    c1.cd();
            ##am    ##AM WIP
            for i in range(3):
            ...
            for i in range(3):
                ...
                pads[i][0].cd();pads[i][0].SetBottomMargin(0.01);pads[i][0].SetTopMargin(0.1);
                pads[i][0].SetLogy()
                plots[i].SetTitle('')
		...
		leg2.Print()
                ...
                pads[i][3].cd()
                pads[i][3].SetTopMargin(0.0005);pads[i][3].SetBottomMargin(0.35);
                ratio_style.Draw("")
	    ...
	    N_4520          = RooRealVar('N_4520_%s'%channel,'N_4520_%s'%channel,cwcbDataHist.sumEntries())
            ...
            for i in range(len(self.POI)):
                ...
                s_name_am = self.POI[i] + '_' + sample + '_' +channel
                fileInHist      = TFile.Open(self.rlt_DIR_name+'/hists4scale_%s_WV_aTGC-%s_%s.root'%(self.ch,self.binlo,self.binhi))
                rrv_x.setRange(self.binlo,self.binhi)                
		...
		#N_quad      = RooRealVar('N_quad_%s'%s_name,'N_quad_%s'%s_name, 0 )
                ...
                if  self.POI[i]=='cb': #sample=='WZ' and
            ...
            cwww_s      = '+@1*(@2/3.6)**2'
            cw_s       = '+@3*(@5/4.5)**2+@4*(@5/4.5)'
            cb_s        = '+@6*(@8/20)**2+@7*(@8/20)'
            cwww_cw_s  = '+@9*(@2/3.6)*(@5/4.5)'
            cw_cb_s    = '+@10*(@5/4.5)*(@8/20)'
