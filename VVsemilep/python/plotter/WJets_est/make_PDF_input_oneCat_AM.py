...
class Prepare_workspace_4limit:
        ...
        def __init__(self,year,ch,mlvj_lo,mlvj_hi,pf=""):
            ...
            self.eps4cbWW               = RooFormulaVar('rel_slope_nuis4cbWW','rel_slope_nuis4cbWW','1+3*(@0-1)',RooArgList(self.eps))
        ...
        def Read_ATGCtree(self,ch='mu'):
            ...
            for WV in ['WW','WZ']:
                ...
                for para in self.POI:
                    ...
                    #hists4scale['c_sm_lin_quad_%s_hist_%s'%(WV,para)]=TH1F('c_sm_lin_quad_%s_hist_%s'%(WV,para),'c_sm_lin_quad_%s_hist_%s'%(WV,para),self.nbins,self.binlo,self.binhi); hists4scale['c_sm_lin_quad_%s_hist_%s'%(WV,para)].Sumw2(kTRUE)
        ...
        def Make_plots(self,rrv_x,cat,fitres):
            ...
            for i in range(3):
                ...
                c       = TCanvas(cat+'_'+self.POI[i]+'-',self.POI[i]+'-',1)
                c.cd()
                pad1        = TPad(cat+'pad1_%s'%self.POI[i],cat+'pad1_%s'%self.POI[i],0.,0.25,1.,1.)
                pad2        = TPad(cat+'pad2_%s'%self.POI[i],cat+'pad2_%s'%self.POI[i],0.,0.02,1.,0.3)
                c2          = TCanvas(cat+self.POI[i]+'+',self.POI[i]+'+',1)
                c2.cd()
                pad3        = TPad(cat+'pad3_%s'%self.POI[i],cat+'pad3_%s'%self.POI[i],0.,0.25,1.,1.)
            ...
            for i in range(3):
                ...
                ##amself.wtmp.data('neg_datahist_%s_%s'%(cat,self.POI[i])).plotOn(plots[i],RooFit.MarkerColor(kBlue),RooFit.LineColor(kBlue),RooFit.DataError(RooAbsData.SumW2),RooFit.DrawOption('E0'),RooFit.Name('atgcdata'))
                ...
                pads[i][0].cd();pads[i][0].SetBottomMargin(0.013);pads[i][0].SetTopMargin(0.1);
                pads[i][0].SetLogy()
                plots[i].SetTitle('')
                ...
                leg.SetFillStyle(0);leg.SetTextFont(42);
                leg.SetBorderSize(0);
                leg.SetFillStyle(0); leg.SetTextSize(0.04);
                ...
                pads[i][1].cd();                pads[i][1].SetTopMargin(0.0005);  pads[i][1].SetBottomMargin(0.35)
                ...
                ratio_style.GetYaxis().SetLabelSize(0.145)
                ratio_style.GetYaxis().SetTitleSize(0.15)
                ratio_style.GetYaxis().SetTitleOffset(0.2)
                ratio_style.GetXaxis().SetLabelSize(0.135)
                ratio_style.GetXaxis().SetTitleSize(0.15)
	    ...
	    N_4520               = RooRealVar('N_4520_%s'%channel,'N_4520_%s'%channel,cwcbDataHist.sumEntries())
            ...
            self.Import_to_ws(self.wtmp,[cwww,cw,cb,self.eps4cbWZ,self.eps4cbWW,SMdatahist,SMdatahist,N_SM])
		...
		if sample=='WW':
            ...
            paralist.add(RooArgList(self.wtmp.function('N_quad_%s_%s'%(self.POI[0],channel)),self.wtmp.function('N_lin_%s_%s'%(self.POI[1],channel)),self.wtmp.var('cwww'),\
                                    self.wtmp.function('N_quad_%s_%s'%(self.POI[1],channel)),self.wtmp.function('N_lin_%s_%s'%(self.POI[1],channel)),self.wtmp.var('cw'),\
            ...
            cwww_s      = '+@1*(@3/3.6)**2' ##+@2*(@3/3.6)'
            cw_s        = '+@4*(@6/4.5)**2+@5*(@6/4.5)'
            cb_s        = '+@7*(@9/20)**2+@8*(@9/20)'
            cwww_cw_s   = '+@10*(@3/3.6)*(@6/4.5)'
            cw_cb_s     = '+@11*(@6/4.5)*(@7/20)'
            Pdf_norm    = RooFormulaVar( 'Pdf_norm_%s'%channel, 'Pdf_norm_%s'%channel, '@0'+cwww_s+cw_s+cb_s+cwww_cw_s+cw_cb_s, paralist)
            paralistN   = RooArgList()
            for i in range(12):
                paralistN.add(RooArgList(paralist.at(i)))
            paralistN.add(RooArgList(Pdf_norm))
            print "AMMMMM",paralistN
            N1                = RooFormulaVar( 'N1_%s'%channel, 'N1_%s'%channel, '@0/@12', paralistN ) #SM
            N2                = RooFormulaVar( 'N2_%s'%channel, 'N2_%s'%channel, '(@1*(@3/3.6)**2)/@12', paralistN )#quad CWWW
            N3                = RooFormulaVar( 'N3_%s'%channel, 'N3_%s'%channel, '(@2*(@3/3.6))/@12', paralistN ) #lin CWWW
            N4                = RooFormulaVar( 'N4_%s'%channel, 'N4_%s'%channel, '(@4*(@6/4.5)**2)/@12', paralistN ) #quad CW
            N5                = RooFormulaVar( 'N5_%s'%channel, 'N5_%s'%channel, '(@5*(@6/4.5))/@12', paralistN )
            N6                = RooFormulaVar( 'N6_%s'%channel, 'N6_%s'%channel, '(@7*(@9/20)**2)/@12', paralistN )
            N7                = RooFormulaVar( 'N7_%s'%channel, 'N7_%s'%channel, '(@8*(@9/20))/@12', paralistN )
            N8                = RooFormulaVar( 'N8_%s'%channel, 'N8_%s'%channel, '(@10*(@3/3.6)*(@6/4.5))/@12', paralistN )
            #N9 ->no aTGC-interference for c_WWW/c_B #FIXME should be added for WZ
            N10                = RooFormulaVar( 'N10_%s'%channel,'N10_%s'%channel,'(@11*(@6/4.5)*(@8/20))/@12', paralistN )

            N_list        = RooArgList(N1,N2,N3,N4,N5,N6,N7)
            N_list.add(RooArgList(N8,N10))
            Pdf_list        = RooArgList(SMPdf)
            ##AMin the following add interference terrm for cwww 
            Pdf_list.add(RooArgList(self.wtmp.pdf('Pdf_quad_cwww_%s'%channel),self.wtmp.pdf('Pdf_lin_cwww_%s'%channel),\
                                    self.wtmp.pdf('Pdf_quad_cw_%s'%channel),self.wtmp.pdf('Pdf_lin_cw_%s'%channel),\
        ...
        def Make_input(self):
            ...
            for region in self.regions:
                ...
                for VV in ['WW','WZ']:
                    ...
                    #pdf_sm_lin_quad_mlvj_VV = self.WS2.pdf('pdf_sm_lin_quad_%s_%s'%(self.ch,VV)) ##AMNew
                    #pdf_quad_mlvj_VV            = self.WS2.pdf('quad_%s_%s'%(self.ch,VV)) ##AMNew
                    #pdf_sm_lin_quad_mj_VV   = w_bkg.pdf('%s_mj_%s_%s'%(VV,region,self.ch))
                    #pdf_sm_lin_quad_VV_2d   = RooProdPdf('sm_lin_quad_%s_%s_%s'%(VV,region,self.ch),'sm_lin_quad_%s_%s_%s'%(VV,region,self.ch),RooArgList(pdf_atgc_mlvj_VV,pdf_atgc_mj_VV))
                    #pdf_quad_VV_2d          = RooProdPdf('quad_%s_%s_%s'%(VV,region,self.ch),'quad_%s_%s_%s'%(VV,region,self.ch),RooArgList(pdf_quad_mlvj_VV,pdf_atgc_mj_VV))
                    #final normalization
                    ...
                    #self.Import_to_ws(self.WS2,[pdf_atgc_VV_2d,signal_norm_VV,pdf_sm_lin_quad_VV_2d,pdf_quad_VV_2d],1)
