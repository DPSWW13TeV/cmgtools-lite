...
processes={
    ...
    'singletop':[ROOT.TColor.GetColor("#9c9ca1"),"single top"],
...
def makePlot(CR,FS,yr,logy):
    ...
    filetoread = ROOT.TFile.Open('fitDiagnosticsTest_fr2_%s.root'%fstr)#_SM_dc_2025-01-21_onelep_%s_%s.root'%(yr,fstr))
    ...
    for ibin in range(1,histdata.GetNbinsX()+1):
        ...
        print('bin ',str(binning[ibin-1]),' ',histInfo['total_background'].GetBinContent(ibin),' pm ',histInfo['total_background'].GetBinError(ibin))
    ...
    band.GetYaxis().SetRangeUser(0.0,2.0 if fit == "fit_s" else 2.5);      band.GetYaxis().SetNdivisions(607);    ROOT.gStyle.SetErrorX(0.5);     band.GetXaxis().SetTitle("m_{WV} (GeV)");
    hist_ratio.SetMarkerStyle(20);   hist_ratio.SetMarkerColor(1);   hist_ratio.SetMarkerSize(1.0);    hist_ratio.SetLineColor(1);  hist_ratio.SetLineWidth(1);
    ...
    legend2.AddEntry(band,"Total unc.","f");
    legend2.Draw("same");
