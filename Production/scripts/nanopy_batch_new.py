...
def batchScriptCERN( runningMode, jobDir, remoteDir=''):
   print("=======================================================================",runningMode,jobDir,remoteDir)
...
""".format(
   ...
   elif remoteDir.startswith("/pnfs/desy.de/"):  #
       print("in DESY")
       cpCmd="""echo '==== sending root files to remote dir ===='
echo
for f in Loop/*.root
do
   ...
   export VO_CMS_SW_DIR=/cvmfs/cms.cern.ch
   source $VO_CMS_SW_DIR/cmsset_default.sh
   env -i X509_USER_PROXY=/afs/cern.ch/user/a/anmehta/private/proxy/myproxy gfal-mkdir "srm://dcache-se-cms.desy.de:8443/srm/managerv2?SFN={srm}" 
   for try in `seq 1 3`; do
      echo "Stageout try $try"
      echo "gfal-copy -n 1 "file:////`pwd`/$f" "{addr}/{srm}/${{ff}}_{idx}.root" "
      env -i X509_USER_PROXY=/afs/cern.ch/user/a/anmehta/private/proxy/myproxy gfal-copy -n 1 "file:////`pwd`/$f" "{addr}/{srm}/${{ff}}_{idx}.root"
      if [ $? -ne 0 ]; then
         echo "ERROR: remote copy failed for file $ff"
         continue
      fi
      echo "remote copy succeeded"
      remsize=$(env -i X509_USER_PROXY=/afs/cern.ch/user/a/anmehta/private/proxy/myproxy gfal-sum "{addr}/{srm}/${{ff}}_{idx}.root" | cut -d" "  -f2) 
      locsize=$(adler32 `pwd`/$f)
      ok=$(($remsize==$locsize))
      if [ $ok -ne 1 ]; then
         echo "Problem with copy (checksums dont match :( ), will retry in 30s"
...
""".format(
          ...
          srm = (""+remoteDir+jobDir[ jobDir.rfind("/") : (jobDir.find("_Chunk") if '_Chunk' in jobDir else len(jobDir)) ]),
          dirCopy = dirCopy,
          addr="srm://dcache-se-cms.desy.de:8443/",
          )
...
class MyBatchManager( BatchManager ):
    def PrepareJobUser(self, jobDir, value ):
       ...
       if self.mode in ('LXPLUS-LSF', 'LXPLUS-CONDOR-SIMPLE', 'LXPLUS-CONDOR-TRANSFER','IFCA','OVD'):
           print(self.mode, jobDir, storeDir)
