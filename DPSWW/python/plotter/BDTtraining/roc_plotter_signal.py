...
outdir='/eos/user/a/anmehta/www/DPSWW_v2/BDT/'
...
for bkg,iset in templates.iteritems():
    ...
    legend = ROOT.TLegend(0.15,0.25,0.25,0.55);
        ...
        for ip in variables:
            for sig in signalMC:
                ...
                if len(ip) == 0:
                    ...
                    #fName='{cs}_{pf}'.format(cs=commonstr,pf=pf)
                else:
                ...
                text4legend.append('{what}{VV}--{when}--{sig}--{bkg}'.format(sig=sig,bkg=bkg,when=str(yr),what='allvars' if len(ip) == 0 else 'allbut-',VV=ip))
                graph.SetLineStyle(signalMC.index(sig)+1);
                ...
                print auc
                text4legend.append('{sig}_vs_{bkg}_{when}[AUC={auc}]'.format(auc=auc,sig=sig,bkg=bkg,when=str(yr),what='allvars' if len(ip) == 0 else 'allbut-',VV=ip))
                #text4legend.append('{sig}_vs_{bkg}_{when}'.format(sig=sig,bkg=bkg,when=str(yr),what='allvars' if len(ip) == 0 else 'allbut-',VV=ip))
    dummy = ROOT.TH2F("dummy","",1000,0,1.0,1000,0,1.0); # for roc
    ...
    c1.SaveAs("{od}/ROC_{here}_AA.png".format(od=outdir,here=bkg));
    c1.SaveAs("{od}/ROC_{here}_AA.pdf".format(od=outdir,here=bkg));
