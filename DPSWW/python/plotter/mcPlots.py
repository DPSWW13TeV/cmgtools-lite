...
from CMGTools.DPSWW.plotter.mcAnalysis import *
import CMGTools.DPSWW.plotter.CMS_lumi as CMS_lumi
...
if "/bin2Dto1Dlib_cc.so" not in ROOT.gSystem.GetLibraries():
    ROOT.gROOT.ProcessLine(".L %s/src/CMGTools/DPSWW/python/plotter/bin2Dto1Dlib.cc+" % os.environ['CMSSW_BASE']);
if "/fakeRate_cc.so" not in ROOT.gSystem.GetLibraries(): 
    ROOT.gROOT.ProcessLine(".L %s/src/CMGTools/DPSWW/python/plotter/fakeRate.cc+" % os.environ['CMSSW_BASE']);
...
def doRatioHists(pspec,pmap,total,maxRange,fixRange=False,fitRatio=None,errorsOnRef=True,ratioNums="signal",ratioDen="background",ylabel="Data/pred.",yndiv=505,doWide=False,showStatTotLegend=False,textSize=0.035):
    ...
    if fitRatio != None and len(ratios) == 1:
        from CMGTools.DPSWW.tools.plotDecorations import fitTGraph
...
#                        if plot.Integral() <= 0: continue
                ...
                legendCutoff = pspec.getOption('LegendCutoff', 1e-5 if c1.GetLogy() else 1e-7)
