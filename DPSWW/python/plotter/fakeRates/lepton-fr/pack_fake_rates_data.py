...
if __name__ == "__main__":
    from CMGTools.DPSWW.plotter.mcEfficiencies import stackEffs, graphFromXSlice
    import os.path
    ...
    parser.add_option("--yrange", dest="yrange", default=(0.0001,0.35), nargs=2, type='float', help="Y axis range");
    ...
    PlotOut="%s/%s/fr-combTest" % ( options.outdir, options.mvaVersionComb)
    ...
    for year in years:
        ...
        for lep,h2d,h2dtt,h2tt_norm,xcuts in (("el",h2d_el,h2d_el_tt,h2d_el_tt_norm,[30]),("mu",h2d_mu,h2d_mu_tt,h2d_mu_tt_norm,[20,45])):
           for ieta,eta in enumerate(["barrel","endcap"]):
               effs = [ (n,graphFromXSlice(h,ieta+1)) for (n,h) in zip(["MC ttbar"],h2dtt) ] #ratio is taken w.r.t this
