...
ttH)
    ...
    CUTFILE="fakeRates/lepton-fr/qcd1l_dpsww.txt"; ;;
...
if [[ "$YEAR" == "2017" ]]; then
    BCORE="${BCORE} --mcc fakeRates/mcc-METFixEE2017.txt "; 
fi;
...
lepdir=${lepton};
case $lepton in
mu) BCORE="${BCORE} -E ^${lepton} --xf 'SingleEl.*,DoubleEG.*,EGamma.*'  "; MVAWP=80; NUM="mvaPt_0${MVAWP}i"; QCD=QCDMu; 
...
Mu3_PFJet40)
    ...
    PUW=" -L fakeRates/lepton-fr/frPuReweight.cc -W 'puw${trigger}_${YEAR}(PV_npvsGood)' "
    ;;
Mu8)
...
MuX_OR)
    ...
    PUW="-L fakeRates/lepton-fr/frPuReweight.cc -W 'coneptw${trigger}_${YEAR}($conept,PV_npvsGood)' "
    ;;
Ele8|Ele8_CaloIdM_TrackIdM_PFJet30)
    ...
    PUW=" -L fakeRates/lepton-fr/frPuReweight.cc -W 'puwEle8_${YEAR}(PV_npvsGood)' "
    ;;
Ele17|Ele17_CaloIdM_TrackIdM_PFJet30)
    ...
    PUW=" -L fakeRates/lepton-fr/frPuReweight.cc -W 'puwEle17_${YEAR}(PV_npvsGood)' "
    ;;
Ele23|Ele23_CaloIdM_TrackIdM_PFJet30)
    ...
    PUW=" -L fakeRates/lepton-fr/frPuReweight.cc -W 'puwEle23_${YEAR}(PV_npvsGood)' "
    ;;
EleX_OR)
...
case $what in
    nvtx)
        echo "python mcPlots.py -f -j 6 $BCORE fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE --sP nvtx $EWKONE " 
        echo "echo; echo; ";
        ...
        ;;
    nvtx-closure)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE --sP nvtx $EWKONE  --showRatio --maxRatioRange 0.9 1.1 " 
        ;;
    coneptw)
        echo "python mcPlots.py -f -j 6 $BCORE fakeRates/lepton-fr/make_fake_rates_xvars.txt --pdir $PBASE --sP ${CONEPTVAR}_nvtx $EWKONE " 
        echo "echo; echo; ";
        echo "python fakeRates/lepton-fr/frConePtWeights.py coneptw${trigger}_${YEAR} $PBASE/make_fake_rates_xvars.root ${CONEPTVAR}_nvtx  ";
        echo "echo; echo ' ---- Now you should put the normalization and weight into frPuReweight.cc defining a coneptw${trigger}_${YEAR} ----- ' ";
        ;;
    coneptw-closure)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/make_fake_rates_xvars.txt --pdir $PBASE --sP ${CONEPTVAR}_nvtx,$CONEPTVAR,nvtx $EWKONE " 
        ;;
    mc-yields)
        echo "python mcAnalysis.py -f -j 6 $BCORE $PUW ${EWKSPLIT} --sp 'QCD.*' --fom S/B --fom S/errSB -G " 
        ;;
    fit-*)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE -E $what $FITEWK --preFitData ${what/fit-/}  $PLOTOPTS " 
        ;;
    num-fit-*)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE -E $what $FITEWK --preFitData ${what/num-fit-/}  $PLOTOPTS -E num" 
        ;;
    num-mcshapes)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE -E $what ${EWKSPLIT/,data/} -E num --plotmode=nostack" 
        ;;
    qcdflav-norm)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE -E $what $QCDNORM --showRatio $PLOTOPTS" 
        ;;
    qcdflav-fit)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE -E $what $QCDFITEWK --preFitData ${what/flav-fit/}  $PLOTOPTS " 
        ;;
    flav-fit*)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE -E $what $QCDFITQCD --preFitData ${what/flav-fit/}  $PLOTOPTS " 
        ;;
    flav3-fit*)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW fakeRates/lepton-fr/qcd1l_plots.txt --pdir $PBASE -E $what $QCDFITALL --preFitData ${what/flav3-fit/}  $PLOTOPTS " 
        ;;
    fakerates-*)
        ...
        STACK="python fakeRates/lepton-fr/stack_fake_rates_data.py "
        ISCOMB=false
        ISWIDE=false
        case $lepton in  
           el) 
               ...
               case $trigger in
                   ...
                   EleX_Combined)
                       ...
                       for E in ${BARREL} ${ENDCAP}; do
                           ...
                           echo "python fakeRates/lepton-fr/combine-fr-bins-prefit.py ${STACK} $PBASE/fr_sub_eta_${E}.root --oprefix ${NUM}_vs_${fitVar}_${XVAR}";
                       done;;
                   EleX_OR)
                       ISWIDE=true
                       XVAR="${CONEPTVAR}"
        ...
        MCEFF="python fakeRates/dataFakeRate.py -f  $BCORE $PUW $EWKONE  --groupBy cut fakeRates/lepton-fr/make_fake_rates_sels.txt fakeRates/lepton-fr/make_fake_rates_xvars.txt  "
        ...
        STACK="python fakeRates/lepton-fr/stack_fake_rates_data.py $RANGES $LEGEND --comb-mode=midpoint" # :_fit
