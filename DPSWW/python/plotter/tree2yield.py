...
from CMGTools.DPSWW.plotter.cutsFile import CutsFile
from CMGTools.DPSWW.plotter.mcCorrections import *
from CMGTools.DPSWW.plotter.fakeRate import *
from CMGTools.DPSWW.plotter.uncertaintyFile import *
from CMGTools.DPSWW.plotter.histoWithNuisances import HistoWithNuisances, cropNegativeBins
    ...
    def getPlotRaw(self,name,expr,bins,cut,plotspec,fsplit=None,closeTreeAfter=False):
        ...
        if canKeys and histo.GetEntries() > 0 and histo.GetEntries() < self.getOption('KeysPdfMinN',2000) and not self._isdata and self.getOption("KeysPdf",False):
            ...
            if "/TH1Keys_cc.so" not in ROOT.gSystem.GetLibraries(): 
                ROOT.gROOT.ProcessLine(".L %s/src/CMGTools/DPSWW/python/plotter/TH1Keys.cc+" % os.environ['CMSSW_BASE']);
            (nb,xmin,xmax) = bins.split(",")
