...
class genInfo_hw_fur_taus(Module):
    def __init__(self):
    ...
    def analyze(self, event):
        ...
        for iGen in genparticles:
            if ( ( abs(iGen.pdgId) in [11,13] and (iGen.statusFlags&(1<<0))==1 and iGen.status == 1  ) or ( abs(iGen.pdgId) == 15 and iGen.status == 2 and (iGen.statusFlags&(1<<0))==1 ) ):
                leptons.append(iGen)
                #else: 
        ...
        return True
#genleptons = lambda : genInfo_hw_fur_taus()
