...
year=${1} 
samples=${2}
steps=("postFSR") #"bdtiv" "recl_allvars") "jme" "lepSFs" "taucount") #"recl") # #postFSR") #
...
do
    ...
    if [[ "${stepToRun}" == "recl" ]];
	then
	python prepareEventVariablesFriendTree.py -t NanoAOD ${baseDir}/${Trees}/${year}/ ${baseDir}/${Trees}/${year}/2_recl/ -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules recleaner_step1,recleaner_step2_mc,mcMatch_seq,higgsDecay,triggerSequence  -d WWDoubleTo2L_newsim -N 60000  -q condor --maxruntime 50 --log $PWD/logs #--de .*Run.*
   ...
   if [[ "${stepToRun}" == "postFSR" ]]; then
	   ...
	   python prepareEventVariablesFriendTree.py -t NanoAOD  ${baseDir} ${baseDir}/postFSRinfo_v1/ -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules genInfo_py8_fur_taus -N 10000  -d WWTo2L2Nu_DoubleScattering_13TeV-pythia8_summer16_nanoV7 -N 100000  #-q condor --maxruntime 100 --log $PWD/logs
	   #python prepareEventVariablesFriendTree.py -t NanoAOD ${baseDir}/${Trees}/${year}/ ${baseDir}/${Trees}/${year}/postFSRinfo/ -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules genInfo_py8_fur_taus --de .*Run.* -N ${nEvt}  -q condor --maxruntime 100 --log $PWD/logs
...
#%%%%%% lepton scale factors 
   ...
   if [[ "${stepToRun}" == "lepSFs" ]]; then
	   python prepareEventVariablesFriendTree.py -t NanoAOD ${baseDir}/${Trees}/${year}/ ${baseDir}/${Trees}/${year}/3_scalefactors_fixed -F Friends ${baseDir}/${Trees}/${year}/2_recl/{cname}_Friend.root -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules leptonSFs  -d WWDoubleTo2L_newsim -N ${nEvt}  -q condor --maxruntime 50 --log $PWD/logs #--de .*Run.* 

   fi
########## tau count

   if [[ "${stepToRun}" == "taucount" ]]; then
	   python prepareEventVariablesFriendTree.py -t NanoAOD ${baseDir}/${Trees}/${year}/ ${baseDir}/${Trees}/${year}/3_tauCount/  -F Friends  ${baseDir}/${Trees}/${year}/2_recl/{cname}_Friend.root  -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules countTaus  -N ${nEvt} -d WWDoubleTo2L_newsim  -q condor --maxruntime 50 --log $PWD/logs #--dm .*forFlips.*
   fi
############ jme 
    if [[ "${stepToRun}" == "jme" ]];    then
	    python prepareEventVariablesFriendTree.py -t NanoAOD ${baseDir}/${Trees}/${year}/ ${baseDir}/${Trees}/${year}/0_jmeUnc_v2/  -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules jme${year}_allvariations  -N ${nEvt} -d WWDoubleTo2L_newsim   -q condor --maxruntime 70 --log $PWD/logs # --de .*Run.*
    fi
#########recl_allvars

  if [[ "${stepToRun}" == "recl_allvars" ]]; then
      echo 'i assume you have already got jme frnds'
	  python prepareEventVariablesFriendTree.py -t NanoAOD ${baseDir}/${Trees}/${year}/ ${baseDir}/${Trees}/${year}/3_recl_allvars/  -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules recleaner_step1,recleaner_step2_mc_allvariations,mcMatch_seq,triggerSequence -F Friends ${baseDir}/${Trees}/${year}/0_jmeUnc_v2/{cname}_Friend.root  -N ${nEvt} -d WWDoubleTo2L_newsim   # -q condor --maxruntime 100 --log $PWD/logs        #--de .*Run.*
