...
do
   ...
   if [[ "${stepToRun}" == "unskimmedbdtDisc" ]]; then
       ...
       python prepareEventVariablesFriendTree.py -t NanoAOD ${baseDir}/${Trees}/${year}/ ${baseDir}/${Trees}/${year}/dpsbdt${pf} -F Friends ${baseDir}/${Trees}/${year}/bdt_input_vars${pf}/{cname}_Friend.root -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules bdtvars2017 -N 100000 -q condor --maxruntime 40 --log $PWD/logs #year needs to be changed for 2016 bdtvars2017
   fi
...
############ everything onwards is post-skimming
  ...
  if [[ "${stepToRun}" == "recl_allvars" ]]; then
   ...
   if [[ "${stepToRun}" == "lepSFs" ]]; then
       python prepareEventVariablesFriendTree.py -t NanoAOD ${baseDir}/${skimmedTrees}/${year}/ ${baseDir}/${skimmedTrees}/${year}/3_scalefactors -F Friends ${baseDir}/${skimmedTrees}/${year}/2_recl/{cname}_Friend.root -I CMGTools.DPSWW.tools.nanoAOD.ttH_modules leptonSFs  -N 70000 --de .*Run.* -q condor --maxruntime 30 --log $PWD/logs 
   fi
